...
import { Upload, RotateCw, Shuffle, Sun, Moon, ArrowLeft, ArrowRight, Check, X, Download, List, Eye, EyeOff } from "lucide-react";
  ...
  for (const r of records) {
    ...
    if (Array.isArray(r)) {
      ...
      if (q != null && a != null && String(q).trim() !== "") {
        out.push({ q: String(q).trim(), a: String(a ?? "").trim() });
      }
      ...
      if (q != null && a != null) {
        ...
        if (keys.length >= 2) {
          ...
          if (q2 != null && a2 != null && String(q2).trim() !== "") {
            out.push({ q: String(q2).trim(), a: String(a2).trim() });
          }
        }
      }
    }
            ...
            <CardContent>
              {started ? (
                <div className="grid gap-4">
                  <div className="grid md:grid-cols-[1fr,300px] gap-4 items-start">
                    <AnimatePresence mode="wait">
                      ...
                      >
                        <div className="text-xs text-neutral-500 mb-2">카드를 눌러 뒤집기 · Space</div>
                          ...
                          <div onClick={() => setFlipped(f => !f)} className="cursor-pointer select-none">
                            {!flipped ? (
                              <div>
                                ...
                                <div className="text-xl sm:text-2xl font-semibold whitespace-pre-wrap break-words">{currentCard?.q}</div>
                              </div>
                            ) : (
                              <div>
                                <div className="text-sm text-neutral-500 mb-2">정답</div>
                                <div className="text-xl sm:text-2xl font-semibold whitespace-pre-wrap break-words">{currentCard?.a}</div>
                              </div>
                            )}
                          </div>
                        </div>
